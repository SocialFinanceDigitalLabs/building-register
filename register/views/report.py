...
def report_json(request):
    ...
    todays_users = User.objects.filter(
        pk__in=SignInRecord.objects.filter(date=for_date).values_list("user", flat=True)
    ).prefetch_related("contactdetails_set").order_by("first_name", "last_name")

    signed_in_users = User.objects.filter(
        pk__in=SignInRecord.objects.filter(date=for_date).open().values_list("user", flat=True)
    )
    context = dict(users=[
        dict(
            first_name=user.first_name,
            last_name=user.last_name,
            emails=list(user.contactdetails_set.filter(method="email").values_list("value", flat=True)),
            phones=list(user.contactdetails_set.filter(method="sms").values_list("value", flat=True)),
            status="signed-in" if user in signed_in_users else "signed-out",
            records=list(user.signinrecord_set.filter(date=for_date).order_by("sign_in__timestamp").values("sign_in__timestamp", "sign_out__timestamp")),
